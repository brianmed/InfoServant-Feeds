...
sub startup {
    ...
    $r->any('/dashboard')->over(params => {new_feed => qr/\w/})->to(controller => 'Dashboard', action => 'new_feed');
    $r->any('/dashboard')->over(params => [qw(opml_file)])->to(controller => 'Dashboard', action => 'opml_file');
